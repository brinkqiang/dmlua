...
public:
    ...
    inline void ReleaseObj(OBJTYPE* tObj)
    {
        ...
        if(!p->dwUse){ abort(); return;}
        assert(p->dwIndex == m_wIndex);
        assert(p >= &m_stRapidData[0] && p <= &m_stRapidData[SIZE-1] && (((char*)p - (char*)&m_stRapidData[0]) % sizeof(SRapidData) == 0));
